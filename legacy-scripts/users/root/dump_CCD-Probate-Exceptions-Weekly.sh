...
# https://tools.hmcts.net/jira/browse/DTSPO-870
set -ue
function log() {
...
OUTPUT_FILE_NAME=CCD-PROBATE-Exceptions-Weekly_${DEFAULT_DATE}.csv
...
function errorHandler() {
  local dump_failed_error="Monday CCD-PROBATE-Exceptions-Weekly report for ${DEFAULT_DATE}"

  log "${dump_failed_error}"

  echo -e "Hi\n${dump_failed_error} today" | mail -s "CCD-PROBATE-Exceptions-Weekly dump ${DEFAULT_DATE} failed in PROD" -r "noreply@reform.hmcts.net (DevOps)" ${FAILURE_ADDRESS}
}
...
trim(CE.data ->> 'surname') AS CE_DECEASED_SURNAME
FROM case_event CE
WHERE CE.case_type_id = 'PROBATE_ExceptionRecord'
...
log "Finished dumping CCD-PROBATE-Exceptions-Weekly query on ${DEFAULT_DATE}"

log "GZIP & Sending email with CCD-PROBATE-Exceptions-Weekly results to: ${TO_ADDRESS} ${CC_LOG_MESSAGE}"

## gzip file before sending
gzip -9 ${OUTPUT_DIR}/${OUTPUT_FILE_NAME}

echo -e "Hi\nPlease find attached CCD-PROBATE-Exceptions-Weekly report for ${DEFAULT_DATE}." | mail -s "CCD-PROBATE-Exceptions-Weekly Reporting" -a ${OUTPUT_DIR}/${OUTPUT_FILE_NAME}.gz -r $FROM_ADDRESS ${TO_ADDRESS} 

log "CCD-PROBATE-Exceptions-Weekly report Complete"
