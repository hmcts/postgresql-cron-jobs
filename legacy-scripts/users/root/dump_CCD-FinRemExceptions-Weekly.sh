...
OUTPUT_FILE_NAME=CCD-FinRemExceptions-Weekly_${DEFAULT_DATE}.csv
...
function errorHandler() {
  local dump_failed_error="Monday CCD-FinRemExceptions-Weekly report for ${DEFAULT_DATE}"

  log "${dump_failed_error}"

  echo -e "Hi\n${dump_failed_error} today" | mail -s "CCD-FinRemExceptions-Weekly dump ${DEFAULT_DATE} failed in PROD" -r "noreply@reform.hmcts.net (DevOps)" ${FAILURE_ADDRESS}
}
...
log "Finished dumping CCD-FinRemExceptions-Weekly query on ${DEFAULT_DATE}"

log "GZIP & Sending email with CCD-FinRemExceptions-Weekly results to: ${TO_ADDRESS} ${CC_LOG_MESSAGE}"

## gzip file before sending
gzip -9 ${OUTPUT_DIR}/${OUTPUT_FILE_NAME}

echo -e "Hi\nPlease find attached CCD-FinRemExceptions-Weekly report for ${DEFAULT_DATE}." | mail -s "CCD-FinRemExceptions-Weekly Reporting" -a ${OUTPUT_DIR}/${OUTPUT_FILE_NAME}.gz -r $FROM_ADDRESS ${TO_ADDRESS} 

log "CCD-FinRemExceptions-Weekly report Complete"
