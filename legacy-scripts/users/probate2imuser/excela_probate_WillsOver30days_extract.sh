...
# https://tools.hmcts.net/jira/browse/PRO-5651

# Alliu Balogun - 05/12/2019
...
DAYSAGO=$(date -d "31 days ago" '+%Y-%m-%d') 
DEFAULT_DATE=$(date +%Y%m%d) 
#DEFAULT_DATE=20190322 
OUTPUT_DIR=/tmp
OUTPUT_FILE_NAME=${DEFAULT_DATE}31daywills.txt
...
function errorHandler() {
  ...
  echo -e "Hi\n${dump_failed_error} today" | mail -s "UK HMCTS Daily Exela extract of Wills over 30 days  ${DEFAULT_DATE} failed in ${environment}" -r "noreply@reform.hmcts.net (DevOps)" ${FAILURE_ADDRESS}
}
...
WITH subquery AS (
  ...
  AND data ->> 'grantIssuedDate' = '${DAYSAGO}'
  --AND data ->> 'grantIssuedDate' between '2019-03-26' and '2019-05-01' 
...
AND state NOT IN ('BOGrantIssued','ScannedRecordAttachedToCase','ScannedRecordManuallyHandled','ScannedRecordReceived') ORDER BY 2) TO STDOUT ( delimiter(','), header false );
EOF
)
...
log "Sending email with Exela  30 day old Report results to: ${TO_ADDRESS} ${CC_LOG_MESSAGE}"


echo -e "" | mail -s "UK HMCTS - Wills older than 30 days" -a ${OUTPUT_DIR}/${OUTPUT_FILE_NAME} -r $FROM_ADDRESS ${CC_COMMAND} ${TO_ADDRESS} 


log "UK HMCTS - Wills older than 30 days extract complete"
