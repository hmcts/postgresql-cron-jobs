...
OUTPUT_DIR=/tmp
OUTPUT_FILE_NAME=AdHocStops${DEFAULT_DATE}.csv
...
AND last_modified::date > '20190326'  ORDER BY 3 ) to stdout with csv header;
EOF
)
...
echo -e "" | mail -s "AdHoc Stops applied to Grants - PRO-6341 " -a ${OUTPUT_DIR}/${OUTPUT_FILE_NAME} -r $FROM_ADDRESS ${TO_ADDRESS} ${CC_ADDRESS} madhu.kumar@justice.gov.uk
