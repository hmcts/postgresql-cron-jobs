...
SELECT DISTINCT
  ...
  data ->> 'deceasedForenames' AS deceasedForenames,
  data ->> 'deceasedSurname' AS deceasedSurname,
...
AND state = 'BOGrantIssued' ORDER BY 3,2) TO STDOUT ( delimiter(','), header false );
EOF
)
