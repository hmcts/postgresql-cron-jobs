...
OUTPUT_DIR=/tmp
OUTPUT_FILE_NAME=${DEFAULT_DATE}_test_wills.csv
...
function errorHandler() {
  ...
  echo -e "Hi\n${dump_failed_error} today" | mail -s "UK HMCTS Will Release ${DEFAULT_DATE} failed in ${environment}" -r "noreply@reform.hmcts.net (DevOps)" ${FAILURE_ADDRESS}
}
...
WITH subquery AS (
  ...
  --AND data ->> 'grantIssuedDate' between '2019-10-01' and '2020-02-17' 
  AND data ->> 'grantIssuedDate' between '2020-02-01' and '2020-02-17' 
  AND dateAdded -> 'value' ->> 'DocumentType' in ('digitalGrant', 'admonWillGrant', 'intestacyGrant'))
  
SELECT DISTINCT
  reference AS ccd_case_number,
  trim(data ->> 'deceasedSurname') AS Surname,
  ...
  trim(data ->> 'deceasedForenames') AS Forename
...
AND state = 'BOGrantIssued' ORDER BY trim(data ->> 'deceasedSurname')) TO STDOUT ( delimiter(','), header false );
EOF
)
...
echo -e "" | mail -s "UK HMCTS Will Release" -a ${OUTPUT_DIR}/${OUTPUT_FILE_NAME} -r $FROM_ADDRESS ${CC_COMMAND} ${TO_ADDRESS} 
